...
def stft(x, fft_size, hop_size, win_length, window):
    ...
    return torch.sqrt(torch.clamp(x_stft.abs() ** 2, min=1e-7)).transpose(2, 1)
...
class Criterion(nn.Module):
    def __init__(self, cfg):
        ...
        # Align STFT params with Audio3DGS forward/iSTFT (n_fft=512, hop=160, win=400, Hamming),
        # otherwise magnitude loss becomes unintentionally sensitive to phase edits.
        self.stft_loss = LogMagSTFTLoss(
            fft_size=512, shift_size=160, win_length=400, window="hamming_window"
        )
        ...
        try:
            self.phase_loss_weight = float(getattr(getattr(cfg, "train", object()), "phase_loss_weight", 0.0))
        except Exception:
            self.phase_loss_weight = 0.0
        # Use a consistent STFT for phase loss too.
        self._phase_fft = 512
        self._phase_hop = 160
        self._phase_win = 400
        self.register_buffer("phase_window", torch.hamming_window(self._phase_win), persistent=False)
    ...
    def forward(self, pred_wav, gt_wav, out_pred_wav_0=None):
        ...
        if pred_wav is not None:
            ...
            scalar_stats['wav_mag_loss'] =  20*self.stft_loss(
                pred_wav.reshape(-1, pred_wav.shape[-1]).contiguous().float(),
                gt_wav.reshape(-1, pred_wav.shape[-1]).contiguous().float()
            ).mean()

            # Optional: phase loss on the unit circle (wrap-around safe).
            # L_phs = ||sin(phi)-sin(phi_hat)||^2 + ||cos(phi)-cos(phi_hat)||^2
            phase_loss = pred_wav.new_tensor(0.0)
            if self.phase_loss_weight > 0.0:
                try:
                    window = self.phase_window.to(pred_wav.device)
                    px = torch.stft(
                        pred_wav.reshape(-1, pred_wav.shape[-1]).contiguous().float(),
                        n_fft=self._phase_fft,
                        hop_length=self._phase_hop,
                        win_length=self._phase_win,
                        window=window,
                        return_complex=True,
                        pad_mode="constant",
                    )
                    gx = torch.stft(
                        gt_wav.reshape(-1, gt_wav.shape[-1]).contiguous().float(),
                        n_fft=self._phase_fft,
                        hop_length=self._phase_hop,
                        win_length=self._phase_win,
                        window=window,
                    ...
                    phi_p = torch.angle(px)
                    phi_g = torch.angle(gx)
                    # unit-circle coordinates
                    sin_p, cos_p = torch.sin(phi_p), torch.cos(phi_p)
                    sin_g, cos_g = torch.sin(phi_g), torch.cos(phi_g)
                    phase_loss = ((sin_p - sin_g) ** 2 + (cos_p - cos_g) ** 2).mean()
                except Exception:
                    phase_loss = pred_wav.new_tensor(0.0)
            scalar_stats["phase_loss"] = self.phase_loss_weight * phase_loss

            # Ensure compatibility with trainers expecting a 'total_loss' key.
            # Here we treat the primary spectral magnitude loss as the total loss.
            scalar_stats['total_loss'] = scalar_stats['wav_mag_loss'] + scalar_stats["phase_loss"]
