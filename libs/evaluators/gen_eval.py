...
def eval_stft(wav):
    """STFT with modern PyTorch API (complex output)."""
    spec = torch.stft(
        wav,
        n_fft=512,
        hop_length=160,
...
class Evaluator:
    def __init__(self, cfg, seq_name, sampling_rate=32000, device='cuda', backbone="cnn14"):
        ...
        # Allow disabling DPAM in training via cfg.train.enable_dpam (default False)
        self.enable_dpam = bool(getattr(getattr(cfg, 'train', object()), 'enable_dpam', False))
    ...
    def evaluate(self, pred_wav, gt_waveform, sr=16000, limit_num=None, recalculate=True, same_name=True):
        # Safe median scale ratio (avoid divide-by-zero due to zero padding/silence)
        pred_abs = np.abs(pred_wav)
        gt_abs = np.abs(gt_waveform)
        # Use non-zero samples for robust median; fallback to global median with epsilon
        gt_nonzero = gt_abs[gt_abs > 0]
        denom = np.median(gt_nonzero) if gt_nonzero.size > 0 else np.median(gt_abs)
        denom = float(denom) if denom is not None else 0.0
        denom = denom if denom > 0 else 1e-8
        numer = float(np.median(pred_abs))
        self.metrics['median_scale'].append(numer / denom)
        ...
        # DPAM (optional, default off during training)
        if self.enable_dpam:
            try:
                # Skip DPAM on degenerate clips
                if pred_wav.shape[-1] <= 0 or gt_waveform.shape[-1] <= 0:
                    pass
                else:
                    loss_fn = cdpam.CDPAM()
                    generate_files_path = f'work_dirs/{self.cfg.output_dir}/tmp_pred'
                    groundtruth_path = f'work_dirs/{self.cfg.output_dir}/tmp_gt'
                    os.makedirs(generate_files_path, exist_ok=True)
                    os.makedirs(groundtruth_path, exist_ok=True)
                    # Ensure finite arrays before writing
                    pred_safe = np.nan_to_num(pred_wav.T, nan=0.0, posinf=0.0, neginf=0.0)
                    gt_safe = np.nan_to_num(gt_waveform.T, nan=0.0, posinf=0.0, neginf=0.0)
                    # Write wavs at the current evaluation sampling rate
                    sf.write(os.path.join(generate_files_path, '1.wav'), pred_safe, sr)
                    sf.write(os.path.join(groundtruth_path, '1.wav'), gt_safe, sr)
                    # cdpam.load_audio internally resamples to 22050
                    wav_ref = cdpam.load_audio(os.path.join(groundtruth_path, '1.wav'))
                    wav_out = cdpam.load_audio(os.path.join(generate_files_path, '1.wav'))
                    dpam_val = loss_fn.forward(wav_ref, wav_out).data.cpu().numpy()[0]
                    self.metrics['dpam'].append(dpam_val)
            except Exception as e:
                # Do not interrupt evaluation on DPAM errors
                pass
        ...
        # Convert complex STFTs to real-valued tensors (last dim of size 2: real, imag)
        if torch.is_complex(pred_spec_tensor):
            pred_spec_tensor = torch.view_as_real(pred_spec_tensor)
        if torch.is_complex(tgt_spec_tensor):
            tgt_spec_tensor = torch.view_as_real(tgt_spec_tensor)
        self.metrics['stft_mse'].append(
            F.mse_loss(pred_spec_tensor, tgt_spec_tensor).detach().cpu().numpy().mean()
        )
